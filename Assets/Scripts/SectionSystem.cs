   ...
   partial struct SectionSystem : ISystem
    {
        ...
        {
            state.RequireForUpdate<CircleRange>();
        }
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            var sectionQuery = SystemAPI.QueryBuilder().WithAll<CircleRange, SceneSectionData>().Build();
            var sectionEntities = sectionQuery.ToEntityArray(Allocator.Temp);
            var circles = sectionQuery.ToComponentDataArray<CircleRange>(Allocator.Temp);
                         ...
                         .WithAll<Relevant>())
            {
                for (int index = 0; index < circles.Length; ++index)
                {
                    ...
                    DrawCircleXZ(circles[index].center + new float3(0f, 0.2f, 0f),
                        circles[index].radius[0], debugColor);
                }
            }
